...
for simulation in range(0,1):
	...
	data = { 	"x":np.zeros([n,2]),\
				...
				"params":{	"n":n,\
							...
							"L":L,\
							"dt":dt}}
		# eactly same as it was from: md_7_multiRun_dictMode.py
	...
	file_x 	= open("a_data_x_K"+str(NSTEP)+"_dt"+str(dt)+"_L"+str(L)+".txt","w") # "_t"+str(time.time())
	file_y 	= open("a_data_y_K"+str(NSTEP)+"_dt"+str(dt)+"_L"+str(L)+".txt","w") # "_t"+str(time.time())
	file_z 	= open("a_data_z_K"+str(NSTEP)+"_dt"+str(dt)+"_L"+str(L)+".txt","w") # "_t"+str(time.time())
	file_vx = open("a_data_vx_K"+str(NSTEP)+"_dt"+str(dt)+"_L"+str(L)+".txt","w") # "_t"+str(time.time())
	file_vy = open("a_data_vy_K"+str(NSTEP)+"_dt"+str(dt)+"_L"+str(L)+".txt","w") # "_t"+str(time.time())
	file_vz = open("a_data_vz_K"+str(NSTEP)+"_dt"+str(dt)+"_L"+str(L)+".txt","w") # "_t"+str(time.time())
	file_fx = open("a_data_fx_K"+str(NSTEP)+"_dt"+str(dt)+"_L"+str(L)+".txt","w") # "_t"+str(time.time())
	file_fy = open("a_data_fy_K"+str(NSTEP)+"_dt"+str(dt)+"_L"+str(L)+".txt","w") # "_t"+str(time.time())
	file_fz = open("a_data_fz_K"+str(NSTEP)+"_dt"+str(dt)+"_L"+str(L)+".txt","w") # "_t"+str(time.time())
	...
	for i in range(0,n-1):
		for j in range(i+1,n):
			...
			xmin = (data["x"][i,0]-data["x"][j,0])-L*round( (data["x"][i,0]-data["x"][j,0]) / L )
			ymin = (data["y"][i,0]-data["y"][j,0])-L*round( (data["y"][i,0]-data["y"][j,0]) / L )
			zmin = (data["z"][i,0]-data["z"][j,0])-L*round( (data["z"][i,0]-data["z"][j,0]) / L )
	...
	for k in range(1,NSTEP):
		...
		for i in range(0,n-1):
			for j in range(i+1,n):
				...
				xmin = (data["x"][i,1]-data["x"][j,1])-L*round( (data["x"][i,1]-data["x"][j,1]) / L )
				ymin = (data["y"][i,1]-data["y"][j,1])-L*round( (data["y"][i,1]-data["y"][j,1]) / L )
				zmin = (data["z"][i,1]-data["z"][j,1])-L*round( (data["z"][i,1]-data["z"][j,1]) / L )
