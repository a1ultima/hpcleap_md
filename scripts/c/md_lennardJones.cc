...
void update_force_positions_velocities();
void update_force();
...
int main(int argc, char *argv[])
{
  ...
  for(int k=1; k<NSTEP; k++){
    ...
    //
    update_force_positions_velocities();

    // Store coordinates at the proposed "initial" for calculating diffusion 
    if(k==1000){
      for(int i = 0; i < N; i++){ 
        xi[i] = x[i];
        yi[i] = y[i];
    ...
    //
    // Calculate variables: temperature, diffusion
    //
    temperature(&T); 
...
void update_force_positions_velocities() //one time step move dt
{ 
  ...
  // FORCE: update forces
  update_force(); // fx, fy and fz stores forces for f(t+1)
